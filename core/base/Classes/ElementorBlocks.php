...
class ElementorBlocks {
	...
	/**
	 * Get an array of all Elementor block IDs with their names.
	 *
	 * @return array Array of Elementor block IDs with names.
	 */
		...
		if ( class_exists( '\Elementor\Plugin' ) ) {
			...
			if ( is_array( $elementor_blocks ) && ! empty( $elementor_blocks ) ) {
				foreach ( $elementor_blocks as $block ) {
					...
					if ( $document && $document->is_built_with_elementor() ) {
						$ids_with_names[ $block->ID ] = get_the_title( $block->ID );
					}
				}
			}
		}

		return $ids_with_names;
	}
}